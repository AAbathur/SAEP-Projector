...
from llava.mm_utils import tokenizer_image_token, tokenizer_image_token_2
...
class ModelArguments:
    ...
    select_multi_layer: Optional[str] = field(default=None)
...
def preprocess_v1(
    ...
    if has_image: # True
        ...
        #    tokenizer_image_token_2(prompt, tokenizer, return_tensors='pt')
        input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                ...
                if round_ids[1] == 3148 and round_ids[2] == 1001:
                    flag = True
                    round_ids = round_ids[:1] + [11889] + round_ids[3:]
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        source[0]['value'] = DEFAULT_IMAGE_TOKEN## 这里把prompt替换成了<image>
        conversation = source[0]['value'] + source[1]['value'] + conversation_lib.default_conversation.sep ## conversation_lib.default_conversation.sep 是 \n
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        list_data_dict = shuffle(list_data_dict, random_state=1123)
        print("****")
        print("list data dict is shuffled with random state 1123")
        print("****")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            if self.data_args.image_aspect_ratio == 'slice': 
                data_dict['h_block'] = h_block
                data_dict['w_block'] = w_block
                data_dict['mode'] = 'slice'
            else:
                data_dict['h_block'] = 1
                data_dict['w_block'] = 1
                data_dict['mode'] = 'single'
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict['image'] = torch.zeros(1, 3, 336, 336)
            data_dict['h_block'] = 1
            data_dict['w_block'] = 1
            data_dict['mode'] = 'text'
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, h_block, w_block = tuple([instance[key] for instance in instances]
                                              for key in ("input_ids", "labels", "h_block", "w_block"))
...
def train():
    ...
    if training_args.gradient_checkpointing: # finetune, True
    ...
    if training_args.lora_enable: ## finetune False
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.freeze_mm_mlp_adapter = training_args.freeze_mm_mlp_adapter ## finetune: false
